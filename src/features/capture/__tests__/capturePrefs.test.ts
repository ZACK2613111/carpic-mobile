...
describe('coerceCapturePrefs', () => {
  it('returns the defaults for an empty / non-object input', () => {
    const d = { fastMode: false, grid: true, level: true, autoCapture: false };
  ...
  it('coerces present-but-corrupt values with Boolean()', () => {
    expect(coerceCapturePrefs({ fastMode: 'yes', grid: 0, level: null, autoCapture: 1 })).toEqual({
      fastMode: true,
      grid: false,
      level: false,
      autoCapture: true,
  ...
  it('keeps a missing key at its default while overriding present ones', () => {
    expect(coerceCapturePrefs({ grid: false })).toEqual({ fastMode: false, grid: false, level: true, autoCapture: false });
    expect(coerceCapturePrefs({ fastMode: true })).toEqual({ fastMode: true, grid: true, level: true, autoCapture: false });
  });

  it('ignores extra keys and always yields booleans', () => {
    const out = coerceCapturePrefs({ foo: 1, bar: 'x', fastMode: 1 } as unknown);
    expect(out).toEqual({ fastMode: true, grid: true, level: true, autoCapture: false });
    expect(typeof out.fastMode).toBe('boolean');
    expect(typeof out.grid).toBe('boolean');
    expect(typeof out.level).toBe('boolean');
    expect(typeof out.autoCapture).toBe('boolean');
